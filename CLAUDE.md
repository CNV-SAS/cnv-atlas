...
- **Hazards de formularios que SOLO aparecen en un navegador real (ni tsc, ni lint, ni jsdom los atrapan).** Son de la misma familia que el de cliente→server-only: verdes en local, rotos en producción o en el navegador. Los conocidos:
  ...
  5. **El `name`/`value` del BOTON de envio NO VIAJA en `new FormData(form)`.** Solo el envio nativo, o `new FormData(form, submitter)`, lo incluyen. Al pasar un formulario por un envio propio (`onSubmit` + `startTransition`), todo boton que lleve su dato en `name`/`value` deja de mandarlo, sin error. Caso real (2026-09-13): "Generar de todos modos" del checkout duplicado mandaba `confirmDuplicate=true` en el boton; no llegaba, el servidor repetia el aviso, y el aviso se volvio un bloqueo. Lo mismo "Confirmar el cargo" / "Rechazar" de un faltante. PRESERVAR: `enviarSinReset` pasa el `submitter` (candado: `enviar-sin-reset-submitter.test.ts`); un envio escrito a mano que lea el boton tiene que pasarlo tambien.
