-- PROMPT DE IA "criterio.generate" v10: publica en BD la version que ya vive en el codigo.
--
-- GENERADO por scripts/gen-ai-prompt-migration.mjs desde supabase/seed.ts, que a su vez lee el JSON canonico
-- src/modules/diagnoses/ai/prompts/criterion.system.v10.json. NO editar a mano: el texto vive en UN sitio y esto se deriva de el.
--
...
UPDATE ai_prompts SET status = 'inactive'
 WHERE prompt_key = 'criterio.generate' AND status = 'active' AND version < 10;
...
SELECT 'criterio.generate', 10, 'Eres el motor clínico del modelo ANI-BIS-E. Escribe el diagnóstico integral del paciente ESTRUCTURADO DESDE EL DIAGNÓSTICO FUNCIONAL INTEGRADO (DFI): los 5 dominios funcionales son el esqueleto del diagnóstico, NO una lista de variables.
...
UPDATE ai_prompts SET status = 'active'
 WHERE prompt_key = 'criterio.generate' AND version = 10
