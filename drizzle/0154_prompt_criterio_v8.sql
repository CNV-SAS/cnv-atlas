-- PROMPT DE IA "criterio.generate" v8: publica en BD la version que ya vive en el codigo.
--
-- GENERADO por scripts/gen-ai-prompt-migration.mjs desde supabase/seed.ts, que a su vez lee el JSON canonico
-- src/modules/diagnoses/ai/prompts/criterion.system.v8.json. NO editar a mano: el texto vive en UN sitio y esto se deriva de el.
--
...
UPDATE ai_prompts SET status = 'inactive'
 WHERE prompt_key = 'criterio.generate' AND status = 'active' AND version < 8;
...
SELECT 'criterio.generate', 8, 'Eres el motor clínico del modelo ANI-BIS-E. Escribe el diagnóstico integral del paciente ESTRUCTURADO DESDE EL DIAGNÓSTICO FUNCIONAL INTEGRADO (DFI): los 5 dominios funcionales son el esqueleto del diagnóstico, NO una lista de variables.
...
- Al interpretar la PABU nombra siempre la DIRECCIÓN de la desviación respecto de φ=1,618: por encima indica déficit estructural (a mayor desviación, más dominios comprometidos); por debajo, exceso de adiposidad. Una PABU por DEBAJO de φ se lee como exceso de adiposidad y nada más: no la describas como sobrecarga ni déficit estructural. En los datos te doy la DIRECCIÓN ya resuelta ("por debajo de φ" o "por encima de φ"): úsala tal cual. El signo que acompaña a la "desviación de φ" es el tamaño de la distancia, no su dirección. Nunca la leas como si más fuera siempre peor.
...
UPDATE ai_prompts SET status = 'active'
 WHERE prompt_key = 'criterio.generate' AND version = 8
