-- PROMPT DE IA "criterio.generate" v12: publica en BD la version que ya vive en el codigo.
--
-- GENERADO por scripts/gen-ai-prompt-migration.mjs desde supabase/seed.ts, que a su vez lee el JSON canonico
-- src/modules/diagnoses/ai/prompts/criterion.system.v12.json. NO editar a mano: el texto vive en UN sitio y esto se deriva de el.
--
...
UPDATE ai_prompts SET status = 'inactive'
 WHERE prompt_key = 'criterio.generate' AND status = 'active' AND version < 12;
...
SELECT 'criterio.generate', 12, 'Eres el motor clínico del modelo ANI-BIS-E. Escribe el diagnóstico integral del paciente ESTRUCTURADO DESDE EL DIAGNÓSTICO FUNCIONAL INTEGRADO (DFI): los 5 dominios funcionales son el esqueleto del diagnóstico, NO una lista de variables.
...
- LA CLASIFICACIÓN QUE ACOMPAÑA A CADA CIFRA, entre paréntesis, ES SU LECTURA: repítela, no la matices ni la contradigas. Si dice "Sobrepeso", es sobrepeso (no "roza el sobrepeso"); si trae una explicación de cómo se lee la escala, síguela. Y escribe cada cifra como te la doy, con sus decimales y su coma decimal.
...
UPDATE ai_prompts SET status = 'active'
 WHERE prompt_key = 'criterio.generate' AND version = 12
