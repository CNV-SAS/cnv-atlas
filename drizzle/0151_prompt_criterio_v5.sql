-- PROMPT DE IA "criterio.generate" v5: publica en BD la version que ya vive en el codigo.
--
-- GENERADO por scripts/gen-ai-prompt-migration.mjs desde supabase/seed.ts, que a su vez lee el JSON canonico
-- src/modules/diagnoses/ai/prompts/criterion.system.v5.json. NO editar a mano: el texto vive en UN sitio y esto se deriva de el.
--
...
UPDATE ai_prompts SET status = 'inactive'
 WHERE prompt_key = 'criterio.generate' AND status = 'active' AND version < 5;
...
SELECT 'criterio.generate', 5, 'Eres el motor clínico del modelo ANI-BIS-E. Escribe el diagnóstico integral del paciente ESTRUCTURADO DESDE EL DIAGNÓSTICO FUNCIONAL INTEGRADO (DFI): los 5 dominios funcionales son el esqueleto del diagnóstico, NO una lista de variables.
...
2) ALERTAS CLÍNICAS DE LA ENCUESTA: el párrafo INMEDIATAMENTE POSTERIOR a la presentación del paciente menciona lo que te doy en el bloque ALERTAS CLÍNICAS, que trae dos cosas. Primero las ALERTAS, con el peso que dice su nivel (crítico primero, luego alto, luego moderado). Después las RESPUESTAS EN ROJO: respuestas del paciente que el clasificador de la encuesta marca en rojo; nómbralas como lo que el paciente respondió, sin convertirlas en un diagnóstico. Nómbralo todo por lo que es, hallazgos de la encuesta, e intégralo en la lectura del caso. Si el bloque dice que no hay ninguna, SÁLTATE este párrafo y NO comentes su ausencia. Si trae alertas positivas, se mencionan al final del mismo párrafo, no antes que lo que hay que atender.
...
UPDATE ai_prompts SET status = 'active'
 WHERE prompt_key = 'criterio.generate' AND version = 5
