...
### Decisión 2026-08-20 v2 (RESPUESTA_GILDARDO + dictamen legal): REVIERTE lo del 15, vuelve a UNA pregunta

La decisión del 15 (dos preguntas + ascendencia + residencia/altitud) **se revierte**: nada de eso estaba parametrizado en el HTML de Gildardo, así que producía datos sin destino. Gildardo (con el dictamen legal a la vista) vuelve a **UNA sola pregunta**, `Etnia / grupo poblacional`, con **7 opciones unificadas para los quince países**: Mestizo/a · Blanco/a · Afrodescendiente · Indígena · Mulato/a · Otro · Prefiero no indicar.

- **Por qué unificar (argumento legal, más fuerte que el de Gildardo):** para operación regional la lista DANE no solo pierde sentido, es **inaplicable**: cada país tiene su marco oficial y no son compatibles entre sí. Una lista armonizada es lo correcto para comparar entre países (sacrifica la comparación con la estadística oficial de cada uno, que queda documentado como límite del dato).
- **Se RETIRAN del intake la pregunta de ascendencia y la de residencia prolongada.** Las columnas (`patient_profiles.ancestry`, `longest_residence_city` en profiles y evaluations) **se conservan** (tienen datos de prueba) pero **quedan SIN USO**: el formulario ya no las envía. Su reintroducción/parametrización va en una ronda con Gildardo (la residencia la pidió el 17 por la altitud fisiológica; su carta del 20 no la menciona).
- **Altitud fisiológica: sin fuente por ahora.** Salía de la residencia prolongada; retirada esa, no hay de dónde derivar la altitud fisiológica. La altitud de la **ciudad actual** mide otra cosa (dónde vive hoy, no dónde se adaptó). Queda como pregunta de la ronda.
- **"Mulato/a": objeción del asesor legal, va a la ronda.** El legal recomienda no usarlo como categoría autónoma (etimología de "mulo"/castas coloniales; carga variable por país, riesgo de que un paciente se sienta agredido; incoherencia de la lista; redundancia con Afrodescendiente) y propone fusionarlo en "Afrodescendiente (incluye negro/a, mulato/a, afrolatino/a)". Se **conserva** por instrucción de la dirección científica; la decisión final es suya. **No hay impedimento legal**: es riesgo reputacional y de calidad del dato, no de legalidad.

**El campo "cuál" del "Otro" (texto libre sobre dato sensible): precauciones NO NEGOCIABLES (dictamen legal 2026-08-20).** El texto libre sobre un dato sensible es la vía más común de fuga: alguien que declara pertenecer a una comunidad pequeña queda identificado con certeza, sin necesidad de su nombre. Por eso:

1. **Límite de 50 caracteres** (UI + `normalizeEtnia` en `validations.ts`): fuerza respuesta categórica, no un relato.
2. **Aviso visible** en el campo: no registrar datos que identifiquen, ni de salud, ni de terceros.
3. **NUNCA como variable de estratificación directa.** Solo puede alimentar análisis DESPUÉS de un proceso de normalización a categorías, revisado por una persona.
4. **EXCLUIDO de toda exportación o vista agregada** hasta que haya sido normalizado. Un reporte con texto libre sin revisar es una fuga esperando ocurrir. (Hoy no hay código de export/observatorio; cuando se construya, honra esta regla: el valor `Otro: <texto>` vive inline en `ethnicity`, el export debe strippear/excluir la parte libre.)
5. Misma protección que el campo estructurado (el régimen de acceso ya definido).
...
## GATE JURÍDICO: el consentimiento v1.0 es colombiano — no operar fuera de Colombia sin adaptación local (dictamen 2026-08-20)

**El punto más importante del dictamen legal del 20, y excede lo que se consultó.** El consentimiento v1.0 está construido **íntegramente sobre derecho colombiano**: cita la Ley 1581 de 2012, la Ley 527 de 1999, las Resoluciones 1995 de 1999 y 839 de 2017, y designa a la **Superintendencia de Industria y Comercio (SIC)** como autoridad de reclamo.

Si el sistema va a operar en quince países, **ese documento no sirve fuera de Colombia sin adaptación**. Cada jurisdicción tiene su propia autoridad de control, sus propios plazos de conservación de historia clínica, sus propios requisitos de autorización para datos sensibles, y sus propias reglas de transferencia internacional. *"Un paciente en Perú al que se le dice que puede reclamar ante la SIC colombiana no tiene un canal de derechos real."*

**GATE (no negociable): antes de operar en cualquier país distinto de Colombia**, el consentimiento y el régimen de tratamiento del dato deben pasar **revisión jurídica local de ese país**. No es algo que resuelva una consulta ni el equipo técnico: requiere abogado local por jurisdicción (autoridad de control, plazos de historia clínica, requisitos de dato sensible, transferencia internacional). Es un asunto de mucho mayor calado que la lista de categorías étnicas, y se aborda **antes** de operar fuera, no después. Relacionado: la unificación de la etnia (arriba) es una condición necesaria pero NO suficiente para operar regionalmente; el consentimiento es el gate mayor.
...
| 16 | 2026-08, corregida 2026-09-13 | **Grafía del nombre de producto = registro sanitario (INVIMA), no la tienda.** Cuando la tienda y el registro difieran, manda el registro (es el nombre con el que el producto está autorizado). **Caso, corregido:** el registro RSA-3987-2026 dice **"MULTI-CELL BASE", con guion** (leído por Santiago en la consulta del INVIMA el 2026-09-13). La primera versión de esta fila afirmaba lo contrario ("el registro dice sin guion") sin haber dejado el texto del registro, y el catálogo se sembró con ese error; lo corrige la migración 0137. **La regla no cambió; el dato estaba mal.** | El nombre autorizado es el del registro sanitario; la página comercial puede tener erratas. Va a volver con cada producto nuevo, y por eso **la grafía se anota con la evidencia del registro** (número y fecha de lectura), no de memoria. |
