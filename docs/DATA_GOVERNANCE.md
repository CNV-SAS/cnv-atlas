...
| 16 | 2026-08 | **Grafía del nombre de producto = registro sanitario (INVIMA), no la tienda.** Cuando la tienda y el registro difieran, manda el registro (es el nombre con el que el producto está autorizado). Caso: la tienda muestra "MULTI-CELL BASE" con guion; el registro dice "MULTICELL BASE" sin guion; el catálogo siembra sin guion. **EN CONFLICTO desde el 2026-09-13:** Santiago informa que el registro (RSA-3987-2026) dice "MULTI-CELL BASE", con guion. El texto del registro no está en el repositorio y no se pudo leer por la consulta pública, así que ninguna de las dos grafías está verificada. No se renombra nada hasta tener el texto del registro. | El nombre autorizado es el del registro sanitario; la página comercial puede tener erratas. Va a volver con cada producto nuevo. |
