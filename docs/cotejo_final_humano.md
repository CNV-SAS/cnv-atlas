...
17. ## URGENTE (subido por Claude)   ·   **CERRADO (2026-09-05)**: tenías razón y era más grave de lo que se veía. Nuestro `clasificarIcaBis` graduaba la desviación en leve/moderada/severa/crítica, y eso es **exactamente lo que Gildardo borró** de su archivo: su fila hace `icaBisClf = cPABU(t_pabu)` y al lado dejó la nota *"cICABIS eliminado, usar cPABU global"*. Su comentario del PABU lo prohíbe de frente: *"La MAGNITUD del deterioro no se gradúa aquí... Duplicar esa graduación en la PABU añadiría bandas sin aportar información nueva"*. La razón que sostenía el nuestro (*"cPABU corta a Reserva superior con IFC>6"*) ya no existe: esa rama se fue con el swap del 18 de agosto y el comentario se quedó citándola. **Y salió un segundo defecto de paso**: la fila ICA-BIS del bloque de índices de la **historia clínica** no podía aparecer nunca, ni desviada ni no, porque el snapshot sella `classifications["ICA-BIS"] = null` y las severidades no emiten esa clave. Ahora la regla vive en UNA función (`conClaseIcaBis`) por la que pasan los dos sitios. Candado en `ica-bis-clase-del-pabu.test.ts` (7 casos, con el control de que sin el alias la fila no aparece). Tu observación original:  "ICA-BIS · Índice de Coherencia Áurea (BIS)	0,42	0 (coherencia)	0,4157	
