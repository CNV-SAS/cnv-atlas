...
- **Extender el mecanismo de grants al resto de la historia clínica identificada (PRIORIDAD ALTA).** El bloque de auditoría/control de calidad cerró el acceso incondicional del `admin` sobre las tres tablas de notas narrativas (`evaluation_notes`, `diagnosis_notes`, `treatment_notes`) y lo reemplazó por el mecanismo de grants (Niveles a/b/c). Queda como brecha conocida el acceso amplio del `admin` por RLS sobre el resto de la historia identificada: `evaluations`, `bis_measurements`, `bis_raw_values`, `diagnoses`, `treatments`, `reports` (incluida la narrativa `reports.professional_notes`, contenido en texto libre del profesional equivalente a las notas ya cerradas, que hoy el admin lee sin grant), `patients`, `patient_profiles`, `survey_responses`/`survey_answers` y demás. Extender la misma infraestructura (`has_active_grant`, `clinical_access_grants`, los tres niveles, la vía de servidor auditada para lo identificado) a esas superficies, cerrando el `has_role('admin')` incondicional donde hoy existe. Documentado como brecha en `SECURITY.md` y `DATA_GOVERNANCE.md`.
...
- **Enriquecimiento visual de la Diana EFR (B12).** El componente ya es funcionalmente correcto (aprobado para el MVP): rejilla polar 9×9 de los 81 estados con la celda del paciente resaltada, accesible y cruzada contra el oro. Diferido, como mejora estética: colorear las 80 celdas restantes por severidad/estado (requiere un mapa de severidad por estado en el registry, hoy inexistente) y hacer el resaltado de la celda del paciente más prominente (aro, elevación o etiqueta flotante). No afecta la corrección del diagnóstico.
