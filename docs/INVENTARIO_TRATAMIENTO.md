...
- **(c) De donde sale:** de `computeDFI`. **CORRECCION 2026-08-22 (pieza 1a.1): NO es "swap wholesale de computeDFI" (eso tocaria el frozen engine.dfi, mas invasivo).** `_parrafo` y `_metaDe` son DERIVADOS: consumen los domains (que el frozen ya produce) + las categorias de indicadores + el fenotipo + el nivel, sin estado privado de computeDFI. Se portan a un MODULO DERIVADO NUEVO (`dfi-narrative`) que consume la salida del DFI sin tocarla; el frozen engine.dfi queda byte-identico (cero riesgo de DIFF-dfi). Porte fiel con golden contra las capturas del HTML, sin modificacion autorizada del frozen. En el re-port de 2026-08-19 solo se porto PABU->Dominio 1; parrafo y metas quedaron fuera.
