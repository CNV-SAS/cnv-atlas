...
14. **Conservación temporal de un borrador de encuesta NO completada (antes de firmar el consentimiento).** Con 63 preguntas y ahora una espera de código por correo en el medio, guardar las respuestas ligadas al token del enlace (sin crear paciente ni evaluación) es lo que decide si el paciente termina o abandona. Pero implica que Atlas tendría **respuestas de salud de una persona que todavía no autorizó nada**. El caso límite es un borrador abandonado: datos de salud de alguien que **nunca completó ni autorizó**, sin titular con quien hablar ni consentimiento que invocar para conservarlos. Por eso la regla de expiración no es higiene: es lo único que hace legítimo el borrador. **Pregunta:** ¿se pueden conservar temporalmente respuestas de salud de una encuesta no completada, antes de que el consentimiento se firme? ¿Por cuánto tiempo, y qué hay que hacer al vencer? **Es el primer paso (gate) del bloque de borrador de encuesta, que va antes del Hito 3.**

13. **HUECO LATENTE detectado 2026-08-09 (no es pregunta, es un defecto a corregir, se registra por su naturaleza legal):** la versión **1.6** del consentimiento se DOCUMENTÓ en `CONSENT_ATLAS.md` (dos finalidades necesarias en el numeral 4: auditoría seudonimizada + acceso excepcional a HC identificada) y es la **base legal de los grants de acceso** (commit `8b88895`), PERO **nunca se portó al texto vendorizado del código** (`consent-v1.5.ts`, `CONSENT_VERSION = "1.5"`). El consentimiento EFECTIVO que firmarían los pacientes NO contiene esas finalidades. → El mecanismo de grants no tiene su base legal en lo efectivamente firmado. **Caracterización completa (auditoría del agente, 2026-08-09):** TRES artefactos tratan v1.6 como vigente mientras el código sirve v1.5 — `SECURITY.md:49` ("su base legal es el numeral 4 del Consentimiento de ATLAS v1.6"), `DATA_GOVERNANCE.md:239` (los grants materializan esas condiciones), y **el comentario de `src/db/schema/enums.ts:141`** (cita v1.6 como base de los topes de grant: el propio código afirma una base que su texto no tiene). Además **rompe la regla C1 del hash:** el numeral 4 del doc tiene 6 viñetas, el del código 4 → el hash se calcula sobre una versión más vieja que la fuente. Y `src/tests/consent-status.test.ts` usa un fixture `"1.6"` mientras producción emite `"1.5"`. **Fix (v1.7):** consolidar v1.6 en el texto vendorizado + actualizar las tres citas de "v1.6" a v1.7 + realinear el test + regenerar el hash. Gate del Hito 3 (con pacientes reales).
