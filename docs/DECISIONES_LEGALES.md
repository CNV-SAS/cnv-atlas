...
10. **Validez de firma electrónica del consentimiento.** El paciente (o su representante legal) acepta el consentimiento en el intake; Atlas ya registra la aceptación con versión, hash del documento, timestamp e IP (tabla `patient_consents`). ¿Eso YA constituye una firma electrónica válida bajo la ley colombiana (Ley 527 de 1999) e internacional, o hace falta un mecanismo/campo adicional (p. ej. captura de firma, OTP, verificación de identidad)? **Bloquea SOLO la parte de "campos de firma / validez" de B7; la parte VISIBLE (ver más, render con datos reales, quitar campos redundantes, asentimiento de menor) avanza sin esto.**
   ...
   - **Sub-pregunta añadida 2026-08-11 (correo del representante en el documento sellado):** el correo del REPRESENTANTE LEGAL aparece en el TEXTO del consentimiento sellado que se firma y se envía (numeral 11 de la instancia; verificado en `consent-instance.ts:207`). El del ADULTO titular NO aparece en el documento (solo nombre y documento). Asimetría que importa: si el correo del representante cambia DESPUÉS de firmar (corrección de un error de digitación, cambio de contacto), el registro (`patient_contacts`) diverge del documento firmado. **Pregunta:** ¿el documento firmado sigue siendo válido con una corrección auditada del correo por fuera, o el cambio obliga a RE-EMITIR (nueva firma/versión del consentimiento)? Para el adulto no se plantea (su correo no está en el documento). No urge (hoy solo demo), pero conviene resuelta antes de pacientes reales: condiciona cómo se construye la corrección diferida del correo (ver `BACKLOG.md`, corrección de datos después de firmar).
...
- **PENDIENTE DE CNV, no del asesor (estado 2026-09-11):** el **número** del registro sanitario de LUVIA está verificado y cargado (INVIMA RSA-0019736-2022, fabricante Laboratorio Naturex S.A.S.), pero **a nombre de quién está el registro NO consta**, y de eso depende si la proveedora responde como titular o revende producto registrado por otro. El **acuerdo escrito con el proveedor sigue abierto a propósito**: se opera como piloto con pocas unidades, y piloto no es acuerdo. Detalle: `MODELO_COMERCIAL_NUTRACEUTICOS_ATLAS.md` §7.7 y §7.8.
