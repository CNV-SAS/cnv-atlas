...
**Ciclo de vida y trazabilidad:** cada grant emite eventos inmutables en el `clinical_audit_log`: `access.requested`, `access.approved` o `access.denied`, `access.used`, y `access.revoked`. Los topes duros (90 días Nivel b, 7 días Nivel c) son **controles de gobierno** que viven en el código (`grant-rules.ts`), no en el consentimiento; su base legal es el numeral 4 del Consentimiento de ATLAS v1.6 (autorización del titular) y las Cláusulas 3 y 17 del Anexo 3 v1.0 (instrucción del Responsable y alcance de la auditoría).
