...
- **Condición 4 — CONFIRMACIÓN EXPLÍCITA (sale del filo del append-only).** Corregir es un acto irreversible más, como confirmar el diagnóstico: una vez insertada la corrección, la evaluación queda reemplazada para siempre (append-only, no se borra ni se edita). Mismo trato que confirmar: una superficie que dice qué va a pasar (se rehace toda la cadena; el anterior queda registrado como reemplazado; si el reporte se envió, la advertencia (b)), exige el motivo, y el profesional confirma entendiendo la consecuencia. Nunca un botón suelto que dispare la corrección sin ese paso.

## El filo del append-only (verificado 2026-08-03, dejar escrito para que no se descubra)

Como la corrección es append-only, una insertada por error (motivo o evaluación equivocada) NO se borra ni se edita, y la evaluación queda reemplazada. Con el servicio completo eso está bien (la dispara un profesional con la Condición 4). Pero hoy la tabla existe SIN servicio.
- **(1a) ¿Hay algún camino que cree una correccion sin querer hoy? NO.** Verificado: nada inserta en `clinical_corrections` (ni seed, ni script, ni código); la única definición es el esquema. Hoy la única vía es SQL directo deliberado.
- **(1b) Doble clic cuando exista el servicio:** el trigger `guard` rechaza corregir una evaluación ya reemplazada, así que el segundo disparo del mismo acto FALLA (la primera corrección ya marcó la vieja como superseded). Compromiso del servicio: ese fallo debe llegarle al profesional como un mensaje claro ("esta evaluación ya fue corregida"), NO el texto técnico del trigger. Idealmente el servicio es idempotente por-intento (deshabilitar el botón tras el primer envío + verificar vigencia antes de escribir).
- **(1c) Salida si una corrección se crea por error:** corregir DE NUEVO la evaluación nueva. La cadena queda `v1 -> v2 -> v3`, con `v2` siendo el error; es trazable y aceptable (nada se pierde, la traza muestra las tres). Esta es LA vía; queda escrita para que no la descubra alguien a mitad de un incidente.
...
### (b) Qué versiones sella la nueva — VERIFICADO, mi recomendación anterior de "fijar el modelo" SE CAE
**Tu límite era el correcto.** Verifiqué (2026-08-03) qué es "el modelo": `buildEngineInput` recibe `model = {version, rulesVersion}`, que son **solo etiquetas**; el motor NO lee parámetros de una tabla. La matemática es el **frozen en disco** (`ENGINE_VERSION = "anibise-1.0.0"`, constante), y lo que se sella como `engine_version` sale de esa constante, no del `model`. Los archivos congelados **no se versionan, se reemplazan** (como hicimos con calcLE8).
- **Consecuencia: "fijar el modelo" NO cambia el cálculo, solo cambia lo que se sella.** Si fijara `model_version_id` al viejo pero corriera el frozen de hoy, el resultado sería el de hoy con una etiqueta que dice otra cosa: **un registro que miente sobre su propia procedencia. Eso es PEOR que no fijar.**
- **Decisión corregida: NO se fija. Se sella la versión REAL (la de hoy: `ENGINE_VERSION`, `emission_versions`, `model_version_id` activos), y se muestra un aviso CONDICIONAL,** solo cuando la versión vieja y la nueva difieren (comparando `engine_version` / `emission_versions` de la vieja vs la nueva): "este diagnóstico se recalculó con una versión posterior del modelo; además del dato que corregiste, pueden haber cambiado otras clasificaciones". Cuando el frozen no cambió entremedio (el caso común), las versiones coinciden y no hay aviso: la corrección aísla la variable igual, sin mentir. Cuando sí cambió, el aviso lo dice y `emission_versions` vieja vs nueva lo hace reconstruible. Es la vía honesta.
...
### Los datos de referencia NO se pueden fijar: la restricción de versión de encuesta (#2 verificado)
Además de los insumos de la evaluación, el pipeline consume DATOS DE BASE: `survey_questions` (el mapeo field_key + texto de opción, leído por la `survey_version_id` de la evaluación) y el contenido del registry (indicator_definitions, texto EFR, sectores FyR, por `model_version_id`). El crítico es la encuesta: **el frozen tiene un acoplamiento char-by-char con el contenido de la encuesta VIGENTE** (le8Total=63, FREQ_GROUPS, los field_key exactos).
- **El problema real y PREVISTO (C9 pasa la encuesta a 15 grupos):** una evaluación vieja respondida con la encuesta anterior, recalculada con el frozen de hoy (acoplado a la encuesta nueva), mezclaría respuestas viejas con un motor que espera otras. `survey_version_id` YA se sella, así que se puede detectar.
- **Decisión: RESTRINGIR, no resolver.** El servicio gatea: solo corrige si `old_eval.survey_version_id == getActiveSurvey().surveyVersionId` (la versión más reciente publicada, contra la que corre el frozen). Si difieren, BLOQUEA con mensaje claro: "esta evaluación se respondió con una versión anterior de la encuesta; no se puede corregir con la versión vigente". **Hoy es un no-op** (el MVP tiene una sola versión de encuesta), pero deja la puerta cerrada para cuando C9 aterrice. El contenido del registry (indicadores/EFR) cae en el aviso condicional de (b), no en esta restricción dura; la encuesta sí es dura por el acoplamiento del frozen.

### Atomicidad (#3 evaluado): el refactor aditivo vs no tocar `writePipeline`
`writePipeline` abre su PROPIA `db.transaction`, y `readPipelineInputs` lee por conexión directa (no vería una copia sin commitear). Dos vías, ambas viables:
- **(3b, RECOMENDADA) refactor ADITIVO de `writePipeline`:** que acepte un `tx` OPCIONAL. `const run = async (tx) => {...cuerpo actual...}; return externalTx ? run(externalTx) : db.transaction(run);`. Cuando NO se le pasa tx, toma EXACTAMENTE el mismo camino que hoy (`db.transaction(run)`): el camino normal queda byte-idéntico, verificable ejecutando (tu 3a: un diagnóstico post-refactor == pre-refactor, con un caso real). Cuando se le pasa, se une a la tx del servicio. Da atomicidad TOTAL: si algo falla, rollback, sin evaluación huérfana. El riesgo es bajo justamente porque es aditivo: el default no cambia.
- **(3c, alternativa sin tocar la función) copia+correccion aparte, pipeline en medio, corrección AL FINAL:** tx1 crea la nueva + copia insumos (commit); corre `runClinicalPipeline` (su propia tx, intacta); tx3 inserta la corrección (que recién ahí marca superseded la vieja). Si el pipeline falla, queda una evaluación nueva SIN diagnóstico y la vieja NO reemplazada (la corrección es lo último): el registro clínico viejo queda intacto, y la huérfana es recuperable (reintentar) o limpiable. Evita tocar la función que escribe todos los diagnósticos, a cambio de un borde de huérfana (y una arruga de lectura: dos evaluaciones no-superseded hasta limpiar).
- **Mi argumento:** (3b) es mejor. Es aditivo (el camino normal no cambia y se verifica idéntico ejecutando, tu propia 3a), y ELIMINA la huérfana por completo. (3c) evita el refactor pero compra un borde recuperable-pero-real. Recomiendo (3b) con la verificación 3a como gate. Si preferís cero cambios a `writePipeline`, (3c) es un fallback válido; es tu llamada.

### Dimensión: es GRANDE, se parte en dos
- **S1 — el motor de la corrección (backend, sin UI de edición):** el gate (asignado + vigente + `survey_version_id` coincide (#2) + motivo + confirmación), la cascada atómica (build-input-desde-la-vieja + aplicar corrección + copiar insumos con verificación exacta + escribir salidas + insertar corrección), la vía de atomicidad elegida (3b aditivo, recomendada), el sellado de la versión REAL + la detección de diferencia de versión para el aviso condicional (b), la invalidación de la aprobación, el audit. Autocontenido y verificable ejecutando (como la migración). YA no incluye "fijar el modelo" (se cayó): es más simple de lo previsto ahí, pero suma el gate de versión de encuesta.
- **S2 — la superficie (UI):** el botón "Corregir" + la confirmación (Condición 4) + el formulario de edición de respuestas + el campo de motivo + la vista de la cadena (e) + los avisos (aviso condicional de versión + invalidación no silenciosa). Se apoya en S1.
- Antropometría/condiciones (superficie) = bloque posterior, reusa S1.

**Recomendación: construir S1 primero** (verificable por ejecución, sin depender de decisiones de UI) y S2 después. **Decisión que necesito de vos antes de S1:** la vía de atomicidad (#3): (3b) refactor aditivo de `writePipeline` con verificación 3a (mi recomendación), o (3c) sin tocar la función aceptando el borde de huérfana. La de modelo (b) quedó resuelta por la verificación (no se fija; se sella la real + aviso condicional).
