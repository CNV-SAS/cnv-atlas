...
- **HUECO DE ALARMA (cuidado b): NO hay.** La magnitud (con el rojo) vive ahora en la fila ICA-BIS via `clasificarIcaBis` (la rama de desviacion del cPABU viejo, ya portada al indicator-ranges), exactamente como dice Gildardo. El profesional ve DIRECCION en la fila PABU (ambar) y MAGNITUD con rojo en la fila ICA-BIS. El caso extremo sigue rojo por ICA-BIS y por los indicadores estructurales (FMI/FFMI/ASMI), donde Gildardo situa la magnitud.
  - **CORREGIDO el 2026-09-05 (cotejo, punto 17): esta conclusion era NUESTRA y estaba mal.** `clasificarIcaBis` no era "la rama de desviacion del cPABU viejo puesta donde Gildardo la situa": era el clasificador que EL BORRO, revivido en la capa de display. Su archivo hace `icaBisClf = cPABU(t_pabu)` con la nota "cICABIS eliminado - usar cPABU global", y su comentario del PABU lo dice literal: "La MAGNITUD del deterioro no se gradua aqui: la establecen los indicadores estructurales... Duplicar esa graduacion en la PABU añadiria bandas sin aportar informacion nueva". O sea que la magnitud SI vive en FMI/FFMI/ASMI, como dice la ultima frase de arriba, y NO en una fila ICA-BIS graduada. `clasificarIcaBis` retirada; la fila toma la clase del PABU via `conClaseIcaBis`. Candado: `ica-bis-clase-del-pabu.test.ts`.
