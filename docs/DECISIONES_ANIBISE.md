...
- Estado: **DECIDIDO / SIN IMPLEMENTAR (verificado 2026-08-03).** El interruptor `LE8_MAPEO_CORREGIDO` en el frozen está en OFF (espeja el vigente, que también lo trae OFF); activarlo es una modificación autorizada del frozen (poner el switch en ON, con bump de versión + re-ancla de golden). **DEPENDENCIA DELICADA:** activar el mapeo cambia el ICEC, que alimenta la EB-BIS; el propio frozen advierte que si la calibración μ/σ de la EB-BIS se hizo sobre el ICEC ya roto, activarlo introduciría una doble corrección. Antes de activar hay que confirmar sobre qué ICEC se calibró (queda como sub-consulta ligada a C2b). · Afecta: frozen `engine.dfi` (ciencia-frozen) + calibración.
