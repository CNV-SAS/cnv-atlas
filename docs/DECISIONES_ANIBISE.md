...
- **HUECO DE ALARMA (cuidado b): NO hay.** La magnitud (con el rojo) vive ahora en la fila ICA-BIS via `clasificarIcaBis` (la rama de desviacion del cPABU viejo, ya portada al indicator-ranges), exactamente como dice Gildardo. El profesional ve DIRECCION en la fila PABU (ambar) y MAGNITUD con rojo en la fila ICA-BIS. El caso extremo sigue rojo por ICA-BIS y por los indicadores estructurales (FMI/FFMI/ASMI), donde Gildardo situa la magnitud.
  - **MATIZADO el 2026-09-05 (cotejo, punto 17), en dos pasadas.** Primera lectura: encontre `icaBisClf = cPABU(t_pabu)` con la nota "cICABIS eliminado - usar cPABU global" y conclui que habia BORRADO el clasificador, asi que retire `clasificarIcaBis`. **Estaba mal**: esa linea es de la HISTORIA CLINICA (ATLAS_v8 L15425). Su archivo tiene DOS reglas para la fila ICA-BIS, una por superficie: en DIAGNOSTICO -> COMPOSICION CORPORAL (L14623) usa `dICA`, que SI gradua la magnitud en cinco escalones, y en la HC usa la del PABU. Lo destapo el cotejo del punto 9, con su captura al lado: en su tabla de Composicion se lee "Desviación leve" en AMBAR.
  - **Asi que el parrafo de arriba se sostiene, con una correccion de color.** La magnitud SI vive en la fila ICA-BIS de esa tabla, como decia. Lo que estaba mal era NUESTRO escalon "Desviación leve" con severidad 0 (verde) donde el suyo es ambar (#f59e0b): un ICA-BIS desviado se pintaba verde al lado de un rotulo que decia que hay desviacion. Ahora la severidad sale del hex de su archivo por `veredictoSev`, el mismo camino de los quince clasificadores congelados. Candado: `ica-bis-dos-reglas-por-superficie.test.ts`.
...
**P-92 · `LE8_MAPEO_CORREGIDO` encendido sin la recalibracion, y las mu/sigma que dijo haber recalculado NO EXISTEN en ninguna entrega suya. BLOQUEA portar `engine.dfi`. Reenviado por mensaje el 3-sep, sin respuesta.** **ATRIBUCION RETIRADA (2026-09-04).** La ronda le atribuia mu=54,306 / sigma=12,845 / 1.847 pacientes. El respondio que esas cifras NO salieron de su Direccion, y las rastreo en su HTML, sus 29 backups, todos sus .md y .docx y el registro de sus sesiones del 2, 3 y 4: su unica aparicion es la linea 120 de NUESTRA ronda. Rastreadas de nuestro lado: vienen de un mensaje interno del 3-sep que se doblo verbatim sin verificar la atribucion. **EL FALLO DE METODO, que es lo que hay que recordar:** se verifico que las cifras no estaban en SU archivo, y no se verifico de donde salia NUESTRA afirmacion de que el las habia dicho. Es la misma verificacion de premisa que si se hizo con lo del "departamento" dos dias despues, un nivel mas arriba. Regla que sale: toda cifra atribuida a la Direccion Cientifica viaja con la entrega o el documento del que salio. Su propio documento lo confirma: su punto 1 cita la llamada con las viejas y donde iba la decision escribio "[PENDIENTE - una de las dos, y va firmada aparte]". Se le piden dos salidas, cualquiera sirve: el archivo con interruptor en true Y las mu/sigma nuevas, o el archivo con el interruptor en false. (Ronda del 2026-09-03, punto 1.) Entre el 1 y el 2 de septiembre el interruptor paso de `false` a `true`. Su comentario, INTACTO en la misma entrega, exige las dos cosas a la vez ("se recalibran en el MISMO acto, nunca por separado") y cierra: "mientras no exista, esta bandera se queda en `false`". El 30 de agosto nos lo habia escrito con las mismas palabras. **Los dos numeros no se movieron:** `_zBis(_icecVal, 58.578, 13.332)` sigue byte por byte igual. Por su propia cifra, encenderlo solo baja la edad bioelectrica de TODOS los pacientes entre 1 y 8 anos.
...
**P-99 · DOS INSTRUCCIONES OPUESTAS EN TRES DIAS sobre la proteina, y la segunda retira de CUATRO modulos lo que la primera mando portar. Ronda del 2026-09-04 punto 3. ENVIADO TAMBIEN POR MENSAJE el 3-sep, sin respuesta.** El 1 de septiembre (§9.6 punto 4) nos mando prescribir la proteina desde el motor y no desde el minimo poblacional; lo portamos el 3 con su cadena de sellado y su tolerancia para los tratamientos anteriores. Su entrega del MISMO 3 retira toda esa prescripcion del motor, y tambien el gasto basal. **NO se porta la retirada.** La asimetria manda: si es deliberada, portarla tarde cuesta un dia; si es un descuido de edicion, portarla deja a Atlas sin la cifra que el acababa de mandar prescribir. La divergencia esta DECLARADA modulo por modulo en `frozen-deriva-vigente` (cuatro toleradas que son UNA decision) y el oraculo de GOLDEN 1 quedo anclado, con fecha y razon, a la entrega del 2. SON CUATRO MODULOS, no uno: `atlas-tratamiento-nutri` (todo `protKg` con sus ramas mas el piso calorico y el tipo energetico), `atlas-protocolo` (la fila renal 0,6-0,8 KDIGO 2024), `engine.core` (las dos recomendaciones de proteina del texto) y `atlas-geb` (`ATLAS_GEB` y `ATLAS_GEB_HB` enteras). **Y LA FECHA CORRECTA DEL §9.6 ES EL 2-SEP, no el 1** (verificado: vive en RESPUESTA_GILDARDO_2026-09-02.md); la ronda lo decia mal y se corrigio antes de enviar. MEDIDO: 60 tratamientos, 56 con 0,8 pasaron al valor del motor y el GEB cambio para todos; **CERO aprobados**, asi que ninguna prescripcion sellada se movio y **Atlas aun no ha atendido a un paciente real** (gates del Hito 3 abiertos). O sea que NO hay nada emitido que corregir, y eso se le dice: baja la urgencia y hace la pregunta contestable. Se le piden DOS cosas: cual rige, y si rige la segunda, si hay que recalcular los 56.
...
**P-101 · Entre seis y ocho subgrupos por region quedan SIN alimento, y su lista impresa saca el rotulo con nada detras. Ronda del 2026-09-04 punto 7.** Su verificacion ("las diez regiones tienen alimentos en los nueve grupos que la prescripcion necesita") es cierta; el hueco esta un nivel mas abajo, en el SUBGRUPO, que es lo que imprime la lista del paciente. Su render no filtra el subgrupo vacio. Medido: Bogota 8, Barranquilla 6, Leticia 7. **Tres se repiten en las diez** (azucares, mecato, bebidas alcoholicas) y se leen como decision suya; los otros no (lacteos descremados vacio en las diez, nueces y semillas en casi todas). La lista COMPLETA no tiene el hueco, asi que viene del recorte y no de INTER_TABLA_B. **Y AL REVISAR EL PDF DE UN PACIENTE (2026-09-04) resulto peor que un rotulo feo: DOS de esos grupos vacios son grupos que el plan PRESCRIBE.** En la misma hoja, la tabla de porciones dice "Azucares y dulces: 1" y tres paginas mas abajo la lista dice "Azucares y dulces:" sin nada detras. Al paciente se le prescribe una porcion de un grupo y se le entrega una lista vacia para elegirla. Medido sobre las diez regiones y tres objetivos caloricos: **"Azucares y dulces" sale prescrito y vacio SIEMPRE**, y **"Nueces" en siete de las diez** (en Caribe, Pacifica e Insular si hay). Eso cambia la pregunta: no es si un rotulo vacio se ve raro, es cual de las dos partes manda (que el reparto no asigne porciones a un grupo que la region no surte, o que la region lleve un alimento de cada grupo que el reparto usa). **Se porta fiel con los rotulos vacios**, porque suprimirlos seria un arreglo de FORMA que taparia un hueco de CONTENIDO. Tres salidas propuestas: salen vacios, se ocultan (y el reparto tampoco les asigna), o los no discrecionales entran al nucleo. Candado: `intercambio-region.test.ts` fija los dos prescritos-sin-lista.
