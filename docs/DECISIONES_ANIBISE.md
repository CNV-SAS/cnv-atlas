...
- Estado: **Parte A (registro) EN CONSTRUCCIÓN 2026-08-08** (tabla + trigger de inmutabilidad + RLS + dominio hechos; falta la UI). **Parte B (redacción "conducta propia")** espera a Gildardo (Q32, con la lista de auto-remisiones por ruta). Afecta: datos (tabla nueva) + frozen (texto de rutas, modificación autorizada).
