...
3. **El DINERO de una devolución: DIFERIDO AL BLOQUE 6 (Santiago, 2026-09-23).** El retracto se activa por venta a distancia (§5.7 del modelo), o sea domicilio, que es el Bloque 6 y no existe: en consulta presencial un sellado que el paciente se llevó no genera derecho de devolución. Se construye con el caso que lo genera, y la consulta a contabilidad se hace entonces. **Lo que sí puede pasar antes es la garantía legal** (producto defectuoso o lote retirado), y para eso el arreglo es chico: la reversa de la sesión 1 ya revierte ingreso y comisión, basta admitir una clase más en su CHECK.
