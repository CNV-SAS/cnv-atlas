...
| 2026-08-04 | `docs/entregas/gildardo-2026-08-04/ATLAS_v8.html` (**PRIMERO que él llama v8, no v7**) | 19.259 | **NADA todavía** (no se ha portado nada; PENDIENTE la prueba de identidad) | `CAMBIOS_ATLAS_v8_para_Santiago.docx` (misma carpeta). Gildardo declara este archivo **VIGENTE** y el v7 (17.923) **CONGELADO**. Trae: cadena de tolerancia del import, lectura de espectroscopía por visión, guarda de `afDesdeCole`, AF a 1 decimal, `REF_POB` (no portar). **Redactado antes de commitear** (URL+clave publishable de Supabase, líneas 29-30). NO re-portar hasta la prueba de identidad |

> **ATENCIÓN (2026-08-04): el archivo VIGENTE es ahora el v8 (19.259), no el v7 del 30-jul (17.923), que Gildardo declara congelado.** Cuando el re-sync/re-port compare "contra el vigente", el vigente es el v8. Riesgo elevado de confusión de nombres: ahora hay TRES archivos y DOS se llaman `ATLAS_v7.html` (16.878 histórico + 17.923 del 30-jul), más el v8. Desambiguar SIEMPRE por ruta + conteo de líneas, nunca por el nombre. Antes de portar nada del v8, **prueba de identidad** (unas líneas citadas por el docx deben caer sobre lo que describe), como se hizo con el del 30-jul.

**CAVEAT sobre la prueba de identidad (2026-07-30): identidad ≠ última versión.** La prueba confirma que este archivo corresponde al **documento del 29** (sus líneas C1-C13 caen donde deben), NO que sea su versión MÁS RECIENTE. Gildardo anunció la corrección de cintura el **30** y dice que va incluida, pero la línea 5600 sigue leyendo el umbral: puede que nos haya enviado su estado del 29, no el del 30. Antes de portar mucho de este archivo, confirmar con él que es su estado ACTUAL (ver `GILDARDO_QUERIES.md` Q18). No confundir "corresponde al doc del 29" con "es lo último que tiene".

**Redacción de secretos (2026-07-30, sidecar — no en el header del HTML, que correría los números de línea).** El `gildardo-2026-07-30/ATLAS_v7.html` traía hardcodeados (2ª vez que pasa con una entrega de Gildardo): clave de Groq `gsk_…` (líneas 12166 y 15215) y credencial de Supabase (URL de proyecto + `sb_publishable_…`, líneas 29-30). Se redactaron **en la misma línea** (`GROQ_API_KEY_REDACTED`, `PROYECTO_SUPABASE_REDACTED`, `sb_publishable_REDACTED`), sin cambiar el conteo de líneas (17.923 antes y después), para no romper la prueba de identidad. Como el secreto ya se había commiteado, se **reescribió el historial** (`rebase --onto`, respaldo creado y borrado tras verificar; commit `bac387c → 5d56d52`) y se purgó de la base de objetos (0 blobs con la clave, verificado). **Decisión: NO se revoca la clave de Groq** (razonamiento en `BACKLOG.md`: el archivo se distribuye para formación, la exposición son las copias que circulan; la clave es de gasto, no de datos; la mitigación real es un límite de gasto en Groq). La clave publishable de Supabase apunta a un proyecto real de Gildardo (observación en `BACKLOG.md` para su entorno).
...
- **`dAECMCA` (AEC/MCA): DESBLOQUEADO (2026-07-30, cambio C12).** Se creía render-only sin cálculo en el frozen. El Decisiones (C12) da la ubicación: **cálculo en `ATLAS_v7:5696`, cortes en `12734`** (`dAECMCA`: <0.45 Óptimo / ≤0.55 Alerta / >0.55 Riesgo). Ya es transcribible verbatim (cálculo + cortes), no autoría. Es una de las filas ausentes de la tabla de Wang (ver el hallazgo de cableado de B11 en BACKLOG); se porta con esa transcripción.
