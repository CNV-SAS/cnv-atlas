...
# 2 · El LE8: se queda encendido, tal cual se envió

**El ICEC se activa tal cual se envió. Esa es la directriz vigente y con ella se cierra el punto.**

El interruptor está en `true` por decisión de esta Dirección, tomada el 2 de septiembre y reafirmada ese
mismo día. No es un descuido, no es un estado intermedio y no es la segunda vez que se les pasa: es la
segunda vez que **no se les dijo**.

## Sobre la nota del 30 de agosto, que es lo que los tiene frenados

**Esa nota se escribió sin haber analizado un solo caso, porque entonces no los teníamos.** Era una
advertencia prudencial, no una medición.

El 2 de septiembre sí se analizaron: se extrajeron los registros reales de ATLAS, se midió el efecto del
mapeo perfil por perfil, y **los ICA-BIS calculados coincidieron exactamente con los guardados en el
sistema**. Con eso a la vista se tomó la decisión de encenderlo.

**Una advertencia escrita sin datos no gobierna sobre una decisión tomada con datos.** El orden correcto
de lectura es ese, y la nota del interruptor queda superada por el acto que la resolvió.

Lo que sigue vale para entender **por qué** se encendió y por qué la recalibración es otra cosa.
...
## La revisión completa de los 350: hecha, y va aquí

No en la próxima entrega: **va en esta**. Se revisaron los 350 nombres, uno por uno.

**Ocho erratas más, ya corregidas** (con las cuatro de la ronda, **doce nombres**):

| Decía | Dice ahora |
| --- | --- |
| **Salmon** rosado, crudo | Salmón |
| Manzana con **cascara** toda variedad | cáscara *(en dos sitios)* |
| Mayonesa **Ligth** | Light |
| Cerveza baja **caloria** 3,2 gr % | caloría |
| **Lcor** de café 21,75 % | Licor |
| Tom **coliins** (ginebra 18,9%) | Tom Collins |
| Avena **instantanea** saborizada | instantánea |
| Ron **33, 4** gr % | 33,4 |

**Y ocho de concordancia y redacción, también corregidas:**

| Decía | Dice ahora |
| --- | --- |
| Avellanas **tostada** sin sal | tostadas |
| Macadamia **tostado** sin sal | tostada |
| Semillas de ajonjolí **tostado** | tostadas |
| Semillas de girasol **descortezado** | descortezadas |
| Chuleta de cerdo **magro** *(en dos sitios)* | magra |
| Queso **búfala** tipo mozzarella | Queso **de** búfala |
| Brevas **almíbar** drenadas | Brevas **en** almíbar |
| Carne de **Cuy o cuy** | Carne de cuy |

**Verificado que ningún cambio de nombre rompió el emparejamiento:** las listas regionales y el núcleo
referencian los alimentos por nombre, y tras las dieciséis correcciones **quedan cero referencias
huérfanas**.

**Y cuatro dudosas que se dejan como están**, por si son marca y no error: *Margarina **line**
(reducida 25%)* —¿light?—, *Brownie **mama-ia** arequipe*, *Seba Seba* y *Taco vacío*. Los nombres
comerciales del mecato (Choco poff, Cheese tris, Tosti arepa, Boli cheetos, Yupis) se escriben como la
marca y tampoco se tocan.

## Y lo que apareció al contar, que no es lo que parece

La tabla tiene **350 filas y 338 nombres**. Los doce nombres que se repiten están todos en Leguminosas
—los nueve fríjoles, el garbanzo, la lenteja y la soya— y hay que decirlo con precisión:

**No son duplicados. Son el mismo alimento en dos tamaños de porción.**

| | Medida | Ejemplo: Fríjol cargamanto blanco |
| --- | --- | --- |
| Primera fila | 1 cucharón colmado | **110 g** |
| Segunda fila | ½ cucharón | **60 g** |

Es contenido deliberado de esta Dirección, y **por eso no se retiró ninguna**.

**El defecto está en el render, no en la tabla.** La lista impresa del paciente muestra
`nombre (gramos)` y **no muestra la medida**, así que las dos filas se leen como el mismo fríjol
repetido. Y hay una consecuencia peor: el render corta en los primeros ocho alimentos de cada subgrupo,
de modo que **las medias porciones ocupan puestos que le quitan variedad real al paciente**.

| Ciudad | Filas de leguminosas | Nombres distintos | Se imprimen |
| --- | --- | --- | --- |
| **Medellín** | 11 | 6 | solo 8, con repeticiones aparentes |
| **Bogotá** | 7 | 4 | las 7 |

Tres salidas, y esta Dirección firma una en la próxima ronda:

1. **El render imprime también la medida** —"1 cucharón colmado" / "½ cucharón"— y la repetición deja de
   parecerlo.
2. **El render agrupa por nombre** y muestra los dos tamaños en una sola línea.
3. **Se retiran las doce medias porciones** de la tabla, si la media porción no aporta nada que el
   nutricionista no pueda indicar por su cuenta.

**No se tocó nada**, y el conteo de la tabla se queda en 350 filas.

**Una última observación, de clasificación y no de ortografía:** en el subgrupo **Leche
semidescremada** hay tres entradas que en su propio nombre dicen otra cosa — *Avena líquida con leche de
vaca **descremada***, *Yogurt de leche **entera** - Yox* y *Yogurt de leche **entera** cuchareable*.
Queda señalado, sin tocar.
...
| **2** | **El ICEC se activa tal cual se envió: el interruptor se queda en `true`.** Es la directriz vigente. La nota del 30 de agosto se escribió sin haber analizado un solo caso; el 2 de septiembre se analizaron los reales y se midió el efecto. **Decisión del 2 de septiembre**, con el defecto que corrige medido. **μ y σ no están pendientes de firma: están bloqueadas por ausencia de ICEC en toda fuente disponible.** Y **μ = 54,306 / σ = 12,845 / 1.847 pacientes no salieron de esta Dirección**: se pide retirar la atribución |
...
| **8** | **Corregidas** — nueve ocurrencias, no cuatro. **La revisión completa de los 350 va en esta entrega**: dieciséis nombres corregidos en total, sin dejar una sola referencia huérfana. **Y las doce filas repetidas de Leguminosas NO son duplicados**: son el mismo alimento en dos tamaños de porción, y el defecto está en el render, que imprime los gramos pero no la medida |
...
| **8** | Las nueve ocurrencias de las cuatro erratas, más ocho erratas y ocho concordancias de la revisión completa: **dieciséis nombres** | Cero apariciones restantes · **cero referencias huérfanas** entre listas regionales, núcleo y tabla · los tres bloques compilan |
