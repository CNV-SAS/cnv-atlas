...
15. **Ninguna evaluación sin las autorizaciones de consentimiento necesarias vigentes.** Las autorizaciones necesarias del gate clínico (`servicio`, `datos_sensibles`) deben existir en `patient_consents` con `revoked_at IS NULL` para el paciente antes de crear cualquier evaluación, inicial o de seguimiento. Si la versión del consentimiento subió de número MAYOR, se requiere re-consentir. Esta verificación vive en la policy `evaluations/can-create-evaluation`, no como chequeo suelto. (Consent v1.0, revisión legal 2026-08-11: eran TRES; `internacional_ia` era acuse de recibo, no autorización, y se absorbió en `servicio`. La aceptación del medio electrónico es requisito de FIRMA, no del gate clínico: la exige `consentSchema` en el servidor y se persiste como registro propio.)
16. **La ciencia congelada no se edita.** Los archivos de `src/clinical-engine/frozen/` (`engine.core.js`, `engine.indices.js`, `engine.dfi.js`) no se editan, no se convierten a TypeScript, no se reformatean y no se corrigen, ni siquiera un bug conocido y confirmado (ver `GILDARDO_QUERIES.md` Q2, TDZ latente aceptado). Son extractos verbatim de la autoría de Gildardo y su fidelidad byte a byte es lo que hace demostrable la regla 6. Exponer funcionalidad que **ya existe** dentro de ellos se hace **únicamente** por el mecanismo de archivo derivado: copia byte a byte del original más una sola línea final aditiva `Object.assign(module.exports, { ... })`, con un test en CI que verifica que el diff contra el original es exactamente esa línea. Ese mecanismo admite **solo exports aditivos, nunca lógica ni fórmulas**. Cualquier cambio a la ciencia que Gildardo entregue como `.js` nuevo entra por swap limpio con golden actualizado (regla 6).

**MECANISMO DE MODIFICACIONES AUTORIZADAS (2026-08-03, para cambios que Gildardo autoriza por INSTRUCCIÓN escrita sin entregar `.js` nuevo).** Cuando Gildardo autoriza retirar o cambiar algo del frozen por instrucción (p. ej. D-012, retirar el examen de telómeros) y no manda un archivo nuevo, NO se edita el original (editarlo pierde para siempre la byte-identidad con su archivo). En su lugar: (1) el original `*.js` queda INTACTO (su DIFF-vs-fuente sigue verde); (2) un MANIFIESTO (`frozen/authorized-modifications.js`, única fuente editable) registra cada modificación como `{ caId, decisión, instrucción verbatim, oldSlice, newSlice }`; (3) un generador determinista produce `*.authorized.js` = original + manifiesto, y ESE es el que corre (los imports apuntan a él); (4) `apply()` exige que cada `oldSlice` aparezca EXACTAMENTE UNA VEZ y prohíbe solapamientos, fallando en voz alta; (5) un test byte-exacto prueba que el generado es el original más EXACTAMENTE esas modificaciones, ninguna más; (6) si cambia el contenido sellado (p. ej. el listado de exámenes va a `protocol_suggested`), sube `PROTOCOL_ENGINE_VERSION`. **Línea de honestidad:** el test prueba que el generado coincide con el manifiesto; que el manifiesto coincida con lo que Gildardo autorizó NO es mecánico, es REVISIÓN HUMANA vía el `caId` y su instrucción verbatim. El mecanismo garantiza trazabilidad y no-adulteración, no corrección clínica. **Vigencia:** al portar un motor nuevo de su archivo, cada modificación se revisa por vigencia (sigue haciendo falta, o Gildardo ya la absorbió y se retira del manifiesto).
...
**NINGÚN VALOR CLÍNICO ENTRA SIN AUTORIZACIÓN EXPLÍCITA, VENGA DE DONDE VENGA — NI SIQUIERA DEL PROTOTIPO DE GILDARDO (lección 2026-08-04).** En la entrega v8, Gildardo detectó que **su propio asistente introdujo SIETE constantes clínicas no aprobadas** en su prototipo (`REF_POB`: hidratación de la MLG 73,2%, reparto del agua 42/58, proteína total 19,4%, etc.) y las marcó explícitamente como "no portar sin validación de la Dirección Científica". Es el MISMO riesgo contra el que se construyó el mecanismo de modificaciones autorizadas (regla 16), ocurriendo del lado suyo: un asistente rellena un hueco con un número plausible y sin firmar, y ese número se volvería ciencia si nadie lo detiene. **Corolario operativo: que un valor aparezca en el HTML de referencia NO lo hace autorizado.** El HTML es autoridad para la LÓGICA portada verbatim, no una vía por la que constantes sin firmar entren al modelo. Cuando el prototipo trae un valor clínico nuevo (un corte, una fracción, una referencia), se verifica que esté APROBADO antes de portarlo; si no consta, se pregunta o se deja el comportamiento conservador (Atlas ya lo hace: la columna Referencia sale de los clasificadores aprobados o queda "-", nunca de una constante poblacional sin firmar). Ver `DECISIONES_ANIBISE.md` P-10.
...
- **El diseño de acceso a contenido clínico se decide por DOS ejes, no uno (corrección del dictamen legal, 2026-08-06).** No basta el riesgo técnico de reidentificación; también pesa lo que el paciente AUTORIZÓ, y la más restrictiva de las dos manda. Caso que lo fija: el equipo propuso ELIMINAR el nivel seudonimizado (ver dato clínico sin nombre) con el argumento, técnicamente correcto, de que quitar el nombre no anonimiza (peso/talla/edad/sexo reidentifican en una red de pocos pacientes). El dictamen concede el argumento pero rechaza la conclusión: el Consentimiento de ATLAS (numeral 4) distingue el control de calidad RUTINARIO sobre dato seudonimizado del acceso IDENTIFICADO excepcional, y les da alcances distintos. Eliminar el nivel seudonimizado habría dejado toda auditoría de calidad bajo el supuesto excepcional, que el paciente NO autorizó para lo rutinario: la medida pensada para proteger más habría dejado a CNV sin base consentida para auditar. **Regla: una decisión de acceso a datos de salud se evalúa contra el riesgo técnico Y contra la autorización del titular, no solo contra el primero.** Ver `PLAN_GRANTS.md` y `docs/entregas/RESPUESTA_CONSULTA_ACCESO_DATOS.md`.
...
- **El protocolo de tratamiento tiene su propio trigger de inmutabilidad (`treatments_immutability`, migración 0026, reducido por la 0116).** Hoy conserva **dos** ramas. (1) `protocol_suggested` es **write-once**: es la salida del MOTOR (regla 7), no una decisión del profesional, y es lo que hace **reproducible** una emisión (la copia guarda los ajustes, y recomputar con ellos solo da lo mismo si el sugerido no se movió). (2) Un tratamiento **con emisiones no se puede borrar**: ya salió hacia una persona. **Lo que se retiró el 2026-09-09 y por qué:** las ramas de *aprobación* (al pasar a `status='approved'` se congelaban `protocol_approved`, los `approved_*`, `kcal_objetivo`, `proteina_g`, los `adj_*` y `micronutrientes_texto`) y de *reapertura*. Ese congelado protegía por **efecto lateral**, no por diseño: el plan impreso, el del correo y la historia clínica se arman los tres del protocolo VIVO, así que solo eran estables mientras nadie reabriera. Lo que conserva **qué recibió el paciente** es ahora `prescription_emissions`, una copia inmutable por cada salida (migración 0115), append-only por trigger propio, y la prescripción queda **siempre editable**. Ver P-120 en `DECISIONES_ANIBISE.md`. **Aviso 2026-08-23: `restrictions_ack_*` NO gatea nada hoy** (ninguna UI invoca la operación y `generateMenu` no la exige): es maquinaria construida y sin cablear, a propósito, porque desde `menu.v2` las restricciones del modelo ya llegan al prompt. **La vía de bypass (`SET LOCAL session_replication_role = replica`) también desactiva estos triggers, con el mismo límite** (solo demo/pre-producción, sin registros clínicos reales). Quien lea que una emisión es inmutable encuentra aquí, en el mismo lugar, que existe esa vía y su acotación.
...
- **Una garantía que vive en el código de aplicación no es una garantía: es una convención. Cuando un invariante importa, la BASE lo hace imposible (lección recurrente, 3ª ocurrencia a 2026-08-07).** Un chequeo en un service protege solo mientras TODO el mundo pase por ese service; una escritura directa, un bug en otra ruta, o un rol con más permiso del previsto lo saltan sin ruido. La defensa real es un trigger o una restricción que rechace el estado inválido pase quien pase. Tres veces apareció el mismo patrón y la solución fue la misma: **(1) el saldo de inventario en consignación** (`nutraceutical_inventory.stock_quantity`) es un cache que SOLO mueve el trigger del movimiento; un trigger de coherencia rechaza cualquier escritura directa que no sea la suma de los movimientos (mig 0040). **(2) `superseded_at`** de las correcciones clínicas: la vieja solo se marca reemplazada por la fila de corrección que la supersede, no por un update suelto (mig 0030). **(3) El gate de dos personas del faltante** (T3b-3): el service restringía a admin a proponer `injustificado_pendiente`, pero el trigger de validez solo exigía `from_status = estado actual`, así que un `en_revision -> injustificado` directo lo saltaba; la mig 0046 agregó la lista de transiciones permitidas y ahora `injustificado` SOLO es alcanzable desde `injustificado_pendiente`, pase quien pase. **Se detectó revisando trabajo ya terminado** (ST1), no al construirlo: el elogio "es imposible saltarse el gate" era falso hasta que se verificó. Corolario: cuando alguien afirme que algo "no puede pasar", verificar DÓNDE vive esa imposibilidad (¿el service, o la BD?); si vive en el service, no puede pasar *todavía*, hasta el primer camino que no pase por ahí.
