...
2bis-b. **El sistema hace algo cuando `complete = false` (ABIERTO, espera Q28).** Hoy `complete = false` **solo MARCA, no impide** emitir el diagnóstico. Con la decisión de que la encuesta es opcional para el paciente y el profesional la completa en consulta, una encuesta recién enviada estará incompleta casi siempre (flujo normal). Qué debe impedir diagnosticar con `complete = false`, y cómo (diagnosticar sin EB-BIS como ya hace sin ICEC / con advertencia / no diagnosticar), es **Q28**, pendiente de la Dirección Científica (`GILDARDO_QUERIES.md`). El gate 2bis NO está cerrado hasta que esta mitad (la que actúa) se resuelva. `BACKLOG.md`.
