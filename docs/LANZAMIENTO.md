...
2bis-b. **El sistema hace algo cuando `complete = false` (ABIERTO, espera IMPLEMENTACIÓN; Q28 ya RESPONDIDA por Gildardo el 2026-08-03).** Hoy `complete = false` **solo MARCA, no impide** emitir el diagnóstico. Gildardo YA decidió qué debe pasar (Q28 cerrada, `GILDARDO_QUERIES.md`): **NO se bloquea**. El diagnóstico bioeléctrico se emite siempre (sale de la medición); lo que depende de la encuesta (índice contextual, edad bioeléctrica, rutas de esos dominios) NO se emite si está incompleta, el profesional ve qué falta y qué queda suspendido, y al completar se genera versión nueva vía el flujo de corrección. Lo que falta es CONSTRUIR ese comportamiento (suspender esas salidas + mostrar lo que falta); es un bloque grande que toca el pipeline, registrado en `BACKLOG.md`. Ya NO espera a Gildardo, espera implementación. **Corolario:** exigir la encuesta completa para diagnosticar contradice esta decisión; no se implementa.
...
**Conteo de gates abiertos: Hito 1 = 4 · Hito 2 = 4 · Hito 3 = 8** (el 2026-08-02 se cerraron el gate 2, bug D8, y 2bis-a, `dfi.complete` mide completitud; queda abierto 2bis-b, la mitad que ACTÚA sobre `complete=false`, que ya NO espera a Gildardo (Q28 respondida 2026-08-03) sino su IMPLEMENTACIÓN; Q8/Q14/P0 cerradas en decisión, pendientes de OK/re-port/trabajo; el gate de opción B se movió de Hito 3 a Hito 2; el gate 19, la lectura funcional del reporte, se movió de BACKLOG). **Total ~19.** Se agregan/mueven conforme aparezcan; cada uno con su puntero al detalle. La matriz rol × hito (mínimo por rol en cada compuerta) se completa aquí cuando se decida por rol.
