...
2bis. **Una encuesta incompleta produce una edad bioeléctrica hasta 21 años mayor, un nivel de riesgo más alto y rutas de atención que no corresponden. Hoy el sistema la sella como completa y no advierte nada.** Verificado experimentalmente con la MISMA medición BIS (`analizarDFI`/`analizarDesdeBiody`, donante anonimizado): con la encuesta a 3/13 campos en vez de 13/13, EB-BIS 29.9 → 50.8 (**+20.9 años**), riesgo MEDIO → ALTO, y se activa una ruta extra (R5 Contextual). La decisión de Gildardo (P0) es que la fórmula NO se toca ni se acota; eso **supone que la encuesta viene completa**. Con una a medias, el sistema inventa un paciente peor del que es, y `dfi.complete` (hoy `= hay al menos un campo`) lo sella como completo sin advertir. Por eso `dfi.complete` **no es un flag de presentación: es lo único que puede impedir que se emita un diagnóstico construido sobre defaults.** El valor es inmutable con el snapshot de `reports`; corregirlo después exige el flujo del gate 1. Debe cerrarse ANTES de sellar diagnósticos reales. Query abierta a Gildardo (¿diagnosticar sin EB-BIS / con advertencia / no diagnosticar?) en `GILDARDO_QUERIES.md`. `BACKLOG.md`.
