...
- **Pregunta a Gildardo:** en la próxima entrega, **exponer `efrProf` en el `module.exports`** (como ya se exponen `getDX`/`efrCompose`), o entregar el abordaje por profesión como **datos** (tabla estado × rol → texto) para poblarlo en el registry como el resto del contenido EFR. Con cualquiera de las dos, Atlas congela el **conjunto completo** de abordajes (los 4 roles) en el snapshot al diagnosticar, sin acoplarse al rol logueado ni al registry vivo (un diagnóstico histórico mostrará el abordaje del rol tal como el modelo lo tenía).
