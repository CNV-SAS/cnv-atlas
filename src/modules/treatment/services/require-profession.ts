...
//
// SOLO filtra null, a proposito. El caso null es inequivoco: sin profesion configurada no se
// prescribe. La MATRIZ de "que profesion puede aprobar que protocolo" (p. ej. si un medico con
// su profesion bien configurada puede aprobar el protocolo NUTRICIONAL del Nivel IV, o solo el
// nutricionista) es GOBERNANZA CLINICA y NO se decide en codigo: es criterio de Gildardo, esta
// abierta en BACKLOG y en la consulta consolidada. No leer este guard como que esa matriz ya se
...
export async function requireConfiguredProfession(actorId: string): Promise<Result<void>> {
  const profession = await getActorProfession(actorId);
  if (!profession) {
    return err(
      appError(
        "forbidden",
