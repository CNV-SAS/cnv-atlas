...
const MIGRACION = "drizzle/0158_prompt_criterio_v12.sql";
...
function generado(): string {
  return execFileSync("node", ["scripts/gen-ai-prompt-migration.mjs", "0158", "criterio.generate"], {
    encoding: "utf8",
...
describe("la migración del prompt se DERIVA del seed, no se escribe", () => {
  ...
  it("el .sql commiteado es EXACTAMENTE lo que el generador produce hoy", () => {
    expect(
      ...
      "el prompt canónico y la migración divergieron: regenera con " +
        "`node scripts/gen-ai-prompt-migration.mjs 0158 criterio.generate > " + MIGRACION + "`",
    ).toBe(norm(generado()));
  });
  ...
  it("y lleva el texto ÍNTEGRO del JSON canónico, no un resumen", () => {
    ...
    const canonico = JSON.parse(
      readFileSync("src/modules/diagnoses/ai/prompts/criterion.system.v12.json", "utf8"),
    ).system as string;
    const sql = generado();
  ...
  it("y NO pisa una edición del admin", () => {
    ...
    const sql = generado();
    expect(sql).toContain("AND version < 12");
