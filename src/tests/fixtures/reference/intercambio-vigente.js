// EXTRACTO VERBATIM de docs/entregas/Gildardo responses/ATLAS_v8.html (2026-08-19): INTER_TABLA_A (L15721-15743),
// INTER_GRUPOS (L15707-15720), y la logica PASO 3 (reparto de porciones por grupo, L16892-16898) envuelta en
// una funcion. Solo para el golden diferencial de intercambio (CP1): NO es codigo de la app. La logica PASO 3
// se copio verbatim de las lineas del componente del v8; repartoGr/subTipo tambien.
const INTER_GRUPOS = [
...
// PASO 3 verbatim (L16892-16898), envuelto: reparto de porciones por grupo desde el objetivo calorico.
function computeIntercambioRef(kcalObj){
  var nx = {};
  var repartoGr = { G1:0.30, G2:0.06, G3:0.10, G4:0.10, G5:0.02, G6:0.12, G7:0.08, G8:0.03, G9:0.11, G10:0.04, G11:0.02, G12:0.02 };
  var subTipo = { G1:"Cereales", G2:"Verduras y hortalizas", G3:"Frutas", G4:"Leche semidescremada", G5:"Sustitutos (embutidos, quesos, huevo)", G6:"Carnes magras", G7:"Leguminosas", G8:"Nueces", G9:"Monoinsaturadas", G10:"Azúcares y dulces", G11:"Mecato", G12:"Bebidas no alcohólicas" };
  INTER_GRUPOS.forEach(function(g){ var sub=subTipo[g.id]; var row=INTER_TABLA_A.find(function(r){return r.sub===sub;}); if(row){ var kcalGr=(kcalObj||0)*(repartoGr[g.id]||0); nx[sub]=Math.max(0, Math.round(kcalGr/row.kcal)); } });
  nx["Verduras y hortalizas"]=2;
  return nx;
}

export { INTER_TABLA_A, INTER_GRUPOS, computeIntercambioRef };
